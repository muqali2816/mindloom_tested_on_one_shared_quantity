...
| File | Contents |
|---|---|
| pair_register_superseded.csv | Pairs removed from the register when a prediction was demoted from stated (M7 pairs with P15, 25 Sep 2026); provenance only. |
...
| Table_S4_v2.csv | Prediction register: 36 entries — 27 stated by proponents, 2 kept for provenance after re-coding to INTERPRETED (P15, P27), 6 derived by the authors (P23, P24, P25, P32, P33, P34) — with source, locus, condition, outcome, expected result, `published_author_statement`, `explicit_endorsement_of_this_test`. |
| column_typology_pairs.csv, column_typology_v2_notes.md | Derived column classes (output of `column_typology.py`; never coded). |
| Table_1_wide_v2.csv, Table_1_tallies_v2.csv | Tallies behind Table 1 (`recompute_table1.py`). |
| Table_S2_v2_experiments.csv | Table S2 — 32 publications in 59 experiment-level rows with `unit_type` (58 experiment, 1 publication-as-one with partition unverified), raw fields `modality`, `affective_status`, `effector_type`, derived `content_class_v2`, three attribution columns, contested flags, evidence status. The publication is the primary unit of Table 2; the experiment-row counts are reported beside it. |
...
| codebook_v2.md, codebook_v2.sha256, codebook_changelog.md | Frozen codebook v2.4. |
...
Coding status at deposit: first coding complete (50 cells added in revision, 24 flagged for adjudication, 2 cells still coded from abstracts — HOT×M6 and PP×M1, sources not obtained — after the full-text reading of 25 September 2026; 10 cells re-coded across 24–25 September with the previous code kept); an independent AI coding against the same codebook is deposited in ai_coder2_check/ as a reproducibility check (κ = 0.55) and is not the second coding; blind second coding by the second author not yet begun — κ will be added in v2.1. Codebook v2.5 frozen (hash in codebook_v2.sha256). DOIs resolved against CrossRef on 24 September
