...
**Response.** §9.1 now specifies one condition completely, and a Supplementary Protocol S5 carries the full protocol for two studies. We should say that the design changed between our first revision draft and this one, for a reason we found ourselves: in the first draft, incompatibility was manipulated through three rules mapped onto the same keys, so that the number of *distinct* responses demanded rose with incompatibility and the two were confounded. That design is replaced.

*Study 1 (neutral content; §9.1 and Protocol S5).* Three responses on three different effectors, all of which can be executed together, so that policy count and incompatibility are separable by construction. Incompatibility is the number of shared-effector pairs among the three demanded responses — 0, 1 or 3 — at a fixed count of three; a count branch varies the number of licensed responses (1, 2, 3) at zero shared pairs. This gives 11 effector-balanced presentation cells and 5 analysis cells. The task, the response mapping, the trial structure, the report-independent access index (an EEG-based index trained on report-session trials and frozen before it is applied to the no-report session — described as a candidate marker of access, not as a measure of M5), the manipulation check, the preregistered exclusions and the recording set (EEG, ECG, electrodermal activity, pupil) are specified. The three confirmatory contrasts are labelled by the prediction they test: C0, the threshold contrast between one and two or more licensed responses at zero shared pairs, tests P16, a prediction *stated* by supramodular interaction theory; C1a, zero versus one or three shared pairs at count three, tests P23, which is *our* extension of that theory to a graded quantity and is labelled as the authors' derivation, not the proponents' claim; C1b is the two-degree-of-freedom omnibus over the three shared-pair levels.

*Power (Protocol S5, `section9_power.py`; all values from the script's output).* For C0 and C1a, N = 119 gives 90 % power at dz = 0.30 (N = 265 at dz = 0.20; N = 68 at dz = 0.40). For the equivalence half of P16 (two one-sided tests, bound ±0.30 dz) power at N = 119 is 0.892 analytic and 0.892 by Monte Carlo (N = 122 for 0.90). For C1b the omnibus power at N = 119 is 0.83 (N = 144 for 0.90). With 25 % probe trials and 15 % expected loss, 102 presented trials per cell yield about 65 usable no-report trials per cell; 561 presented trials per session over the 11 presentation cells; two sessions. A mixed-model check with random slopes (200 simulated data sets per contrast) gives 0.93 and 0.92 against the analytic 0.90, with no non-converged fits. N is fixed in advance; there is no sequential monitoring. The cortical–autonomic coupling index is preregistered as *estimation only*; it is not a test of M5.

*Study 2 (valence; Protocol S5 only).* The main effect negative − positive at matched arousal at dz = 0.30 needs N = 119. The incompatibility × valence interaction at dz = 0.20 would need N = 265; at N = 119 its power is 0.58, so it is reported as an estimate with its interval, not tested.

**Status:** DONE.
**Location of change:** §9 Conclusion (design paragraph); §9.1 One worked condition; Supplementary Protocol S5; deposit files `section9_power.py`, `power_study1.csv`, `power_study2.csv`, `trials_budget.csv`.
...
**(a) M8 was called shared and treated as non-discriminating; it is contested.** The submitted version, and our first revision draft, described the cortical signature of access as a column on which predictions converge, and drew the conclusion that an experiment on it "tests many and discriminates none". When the stated predictions were entered into the register and compared pairwise, M8 came out contested by the rule of the codebook: six accounts state predictions and 15 pairs of stated predictions are distinguishable — at the locus of decodable content, at the timing of ignition versus sustained posterior activity, and at fronto-posterior versus within-posterior connectivity. Cogitate preregistered exactly those divergences and discriminated on them (IIT passed duration and failed sustained posterior synchrony; GNWT found prefrontal category decoding but neither offset ignition nor identity decoding). The correct statement, now in §6.4, §7.4 and §9, is that the M8 predictions share the *quantity* and are contested at locus, timing and connectivity — which is why Cogitate could test both theories and did produce differential results, and why the field's tests concentrate there. The sentence that a shared prediction cannot discriminate has been withdrawn, and the argument of the paper no longer rests on it: it rests on the concentration of stated commitments on a few quantities and on the columns nobody has put a prediction on.

**(b) M4 split into M4a, M4b, M4c; "the only contested column" withdrawn.** "Autonomic/interoceptive channel involvement" conflated three quantities: access to interoceptive content as content (M4a), visceral modulation of access to exteroceptive content (M4b), and conscious involvement in a purely autonomic conflict (M4c). Once separated, the stated null of supramodular interaction theory and passive frame theory is a prediction over M4c (and, on our reading of the primary text, an exclusion from the *function* of the conscious field rather than a denial that bodily feelings exist — the manuscript's wording has been corrected accordingly), while the positives of predictive processing, the neural subjective frame and the Feinberg–Mallatt scheme are predictions over M4a and M4b. No pair of stated predictions is distinguishable within any of the three, so all three are *occupied-not-contested* by the derived rule. The claim that M4 is the only contested column, and the abstract's description of it as carrying opposed predictions, are withdrawn. What would make M4c contested is a stated interoceptive-inference prediction that a purely autonomic conflict is felt; we have derived it (P24) and labelled it as our derivation pending the proponents. The 36 M4a/b/c cells are provisional pending the second coding. This changes what §9 says about "the condition that discriminates cannot be run": it is now "the condition that *would* discriminate, if the proponents state the prediction we have derived for them, cannot yet be run".

**(c) Table S2 rebuilt at experiment level; paradigm papers out of the denominator; attribution split.** Described at R1-8. The consequences for the numbers: the submitted 21 of 27 (publication level, paradigm-defining rows inside the denominator) becomes 18 of 30 experiments at baseline, with the neutral-visual share ranging from 10 of 21 (strict attribution, all contested rows removed) to 22 of 35 (coder inference admitted) across the sensitivity scenarios; the interoceptive cell is 2 of 30 rather than 0; the valenced cell is 0 of 30 at baseline and 1 of 31 or 3 of 39 under alternative readings. The statement "no theory claims this literature" is withdrawn and replaced by a statement about the checked sample under the stated rule.

**(d) §9.1 redesigned; N recomputed.** Described at E5. The first revision draft's design (three rules over the same keys, N = 126, sequential Bayes-factor monitoring) confounded the number of distinct responses with incompatibility and is replaced by three co-executable responses on three effectors, with incompatibility as the number of shared-effector pairs (0/1/3) and a separate count branch. N = 119 for 90 % power at dz = 0.30 on C0 and C1a; no sequential monitoring; valence moved to Study 2 in Protocol S5; the coupling index and the interaction preregistered as estimation only. We also withdrew the sentence that "three quantities from Table 1 then become jointly observable for the first time", which the redesigned condition does not support.

**(e) Codebook v2.0 frozen with hash; second coding planned with partial blindness stated.** Described at R1-5. The freeze (24 September 2026, SHA-256 in the footer) precedes the second coder's start; the pack sent to the second coder contains no first-coder codes; the limit on blindness — the second author has read the submitted manuscript's condensed Table 1 — is stated in §7.2 and agreement will be reported separately for the cells whose codes appeared in the submitted version and for the cells that did not exist then.
