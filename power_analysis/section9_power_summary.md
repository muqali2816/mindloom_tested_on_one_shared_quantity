# Section 9.1 power analysis, new design (section9_power.py v2.3)
...
**C0 (P32, author-derived threshold: count 1 vs >= 2 at 0 shared pairs; weights [-1.0, 0.5, 0.5]).** N = 119 participants are required for 90 % power at dz = 0.3 (achieved 0.901); N = 265 at dz = 0.20 and N = 68 at dz = 0.40.
**C1a (P23, author-derived: 0 shared pairs vs mean of 1 and 3 shared pairs at count 3; weights [-1.0, 0.5, 0.5]).** The same paired-t computation applies: N = 119 at dz = 0.3. The fixed sample is N = 120 (the required 119 rounded up to a multiple of the 6 rule-to-effector rotations); its minimum detectable dz at 90 % power is 0.298.
**Equivalence half, P33 (count 3 - count 2, TOST).** At N = 120 and a true effect of zero, exact TOST power is 0.406 (Monte Carlo 0.406 +/- 0.001) for bounds +/-0.2; 0.895 (Monte Carlo 0.896 +/- 0.001) for bounds +/-0.3; 0.993 (Monte Carlo 0.993 +/- 0.000) for bounds +/-0.4. To reach 90 % TOST power one would need N = 272 at +/-0.2, 122 at +/-0.3, 70 at +/-0.4. The bound +/-0.3 is the planning effect size itself; the equivalence claim is thus adequately powered at the fixed N only for that bound or wider.
**C1b (0 / 1 / 3 shared pairs, unequal steps).** With weights linear in the number of shared pairs ([-1.3333, -0.3333, 1.6667], i.e. -4, -1, +5 up to scale) the one-df test needs N = 265 at dz = 0.2, 119 at dz = 0.3, 68 at dz = 0.4. The two-df omnibus (Hotelling T^2, effect on one contrast direction) needs N = 320 at dz = 0.2, 144 at dz = 0.3, 83 at dz = 0.4; at the fixed N = 120 its power is 0.476 at dz = 0.2, 0.836 at dz = 0.3, 0.979 at dz = 0.4. The ordinal weights (-1, 0, +1) are not used: they would treat the step from 1 to 3 shared pairs as equal to the step from 0 to 1.
