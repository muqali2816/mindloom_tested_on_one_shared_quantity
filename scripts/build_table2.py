#!/usr/bin/env python
"""
build_table2.py (v2.1) -- Table 2 counts from Table_S2_v2.csv at BOTH units (audit section 7).
...
def publication_class(classes: set[str], pub_id: str) -> str:
    if len(classes) == 1:
...
def build(rows):
    ...
    for pid, prs in pubs.items():
        ...
        pub_info[pid] = dict(cls=publication_class(classes, pid), ta=any(B(r["theory_addressed"]) for r in prs), whalen=any(is_whalen(r) for r in prs),
